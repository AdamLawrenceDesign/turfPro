...
OverlayBuilder.prototype.buildCropMark = function()	
{
	...
	$(obj).attr('id','cropMark').css({'display':'none','position':'absolute','pointer-events':'none','margin':'auto','z-index':'100','border':'1px dashed #ddd' }).addClass('absolute_vert_center'); 	// 'width': width, 'height': height
...
$(function()
{
	...
	$.ajax(
	{
		url: 'http://192.168.0.216/AdvAPI/api/WCAPValues/Photocreate/' + itemName + '/' + id,
		type: 'GET',
		username: 'WebAPIPhotocreateUser',
		password: '@dvw3b@piu$3r',
		success: function(data)
	...
	$('header').on('click', '#export', function()
	{
		...
		// window.location = 'http://192.168.0.177/_testing/proto2/payment.html';
	});
	*/
});
